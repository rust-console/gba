...
/// An optimization to allow us to short circuit debugging early when there is
/// no interface.
static NO_DEBUG: Static<bool> = Static::new(false);
/// The debugging interface in use.
static INTERFACE: Static<Option<&'static dyn DebugInterface>> =
  Static::new(None);
...
#[inline(never)]
pub fn debug_print(
  debug: DebugLevel, args: &Arguments<'_>,
) -> Result<(), Error> {
