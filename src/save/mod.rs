...
//!
//! This module provides both specific interfaces that directly access
//! particular types of save media, and an abstraction layer that allows access
//! to all kinds of save media using a shared interface.
//!
//! ## Save media types
//!
...
//! calling the [`prepare_write`](`SaveAccess::prepare_write`) method before
//! doing the actual write commands with the [`write`](`SaveAccess::write`)
//! method.
//!
//! ```rust
//! # use gba::{info, save::SaveAccess};
...
//! a range of `0..8192`. Use [`sector_size`](`SaveAccess::sector_size`) to find
//! the sector size, or [`align_range`](`SaveAccess::align_range`) to directly
...
//!
//! * SRAM does not require `prepare_write` and has no sectors to align to.
//!   Reads and writes at any alignment are efficient. Furthermore, it does not
//!   require a timer to be set with [`set_timer_for_timeout`].
//! * Non-Atmel flash chips requires `prepare_write`, and have sectors of 4096
//!   bytes. Atmel flash chips instead do not require `prepare_write`, and
//!   instead have sectors of 128 bytes. You should generally try to use
//!   `prepare_write` regardless, and write in blocks of 128 bytes if at all
//!   possible.
...
static CURRENT_SAVE_ACCESS: Static<Option<&'static dyn RawSaveAccess>> =
  Static::new(None);
...
impl SaveAccess {
  ...
  /// Returns whether this save media requires the use of
  /// [`SaveAccess::prepare_write`].
  ...
  ///
  /// If [`requires_prepare_write`](`SaveAccess::requires_prepare_write`)
  /// returns `true`, you must call
  /// [`prepare_write`](`SaveAccess::prepare_write`) on the range you intend
  /// to write for this to function correctly. The contents of the save media
  /// are unpredictable if you do not.
  ...
  /// [`prepare_write`](`SaveAccess::prepare_write`) on the range you intend
  /// to write for this to function correctly. The contents of the save media
  /// will be unpredictable if you do not.
  ///
