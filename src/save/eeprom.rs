...
const PORT: VolAddress<u16, Safe, Safe> =
  unsafe { VolAddress::new(0x0DFFFF00) };
...
impl EepromProperties {
  ...
  fn write_sector_safe(
    &self, word: usize, data: &[u8], start: usize,
  ) -> Result<(), Error> {
...
const PROPS_512B: EepromProperties =
  EepromProperties { addr_bits: 6, byte_len: 512 };
const PROPS_8K: EepromProperties =
  EepromProperties { addr_bits: 14, byte_len: 8 * 1024 };
