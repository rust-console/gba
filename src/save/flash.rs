...
use super::{
    Error, RawSaveAccess, MediaType, MediaInfo,
    read_raw_buf, read_raw_byte, verify_raw_buf, lock_media,
};
...
fn cached_chip_info() -> Result<&'static ChipInfo, Error> {
    CHIP_INFO.try_get(|| -> Result<_, Error> {
...
impl ChipInfo {
    ...
    fn read_buffer(&self, mut offset: usize, mut buf: &mut [u8]) -> Result<(), Error> {
        while buf.len() != 0 {
            ...
            unsafe { read_raw_buf(&mut buf[..end_len], 0x0E000000 + offset); }
            buf = &mut buf[end_len..];
            offset += end_len;
        }
        Ok(())
