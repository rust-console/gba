...
use super::{
  lock_media, read_raw_buf, read_raw_byte, verify_raw_buf, Error, MediaInfo,
  MediaType, RawSaveAccess,
};
...
const FLASH_PORT_BANK: VolAddress<u8, Safe, Safe> =
  unsafe { VolAddress::new(0x0E000000) };
const FLASH_PORT_A: VolAddress<u8, Safe, Safe> =
  unsafe { VolAddress::new(0x0E005555) };
const FLASH_PORT_B: VolAddress<u8, Safe, Safe> =
  unsafe { VolAddress::new(0x0E002AAA) };
const FLASH_DATA: VolBlock<u8, Safe, Safe, 65536> =
  unsafe { VolBlock::new(0x0E000000) };
...
fn cached_chip_info() -> Result<&'static ChipInfo, Error> {
  CHIP_INFO
    .try_get(|| -> Result<_, Error> {
      Ok(FlashChipType::detect()?.chip_info())
    })
    .map(Clone::clone)
}
...
impl ChipInfo {
  ...
  fn check_sector_len(&self, offset: usize, len: usize) -> Result<(), Error> {
    if offset.checked_add(len).is_some()
      && offset + len <= self.info.sector_count
    {
      Ok(())
    } else {
      Err(Error::OutOfBounds)
  ...
  fn read_buffer(
    &self, mut offset: usize, mut buf: &mut [u8],
  ) -> Result<(), Error> {
    while buf.len() != 0 {
  ...
  fn verify_buffer(
    &self, mut offset: usize, mut buf: &[u8],
  ) -> Result<bool, Error> {
    while buf.len() != 0 {
      self.set_bank(offset >> BANK_SHIFT)?;
      let start = offset & BANK_MASK;
  ...
  fn wait_for_timeout(
    &self, offset: usize, val: u8, ms: u16,
  ) -> Result<(), Error> {
